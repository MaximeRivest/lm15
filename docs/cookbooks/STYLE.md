...
7. Errors shown deliberately (recipe 17) follow the same rules: real
...
- **Models.** Default to cheap: `gpt-4.1-mini` (OpenAI),
  ...
  appears only in recipe 16 and is guarded by a reachability note, not
  try/except.
...
- Pages live in `docs/cookbooks/`, named `NN-slug.md` (see
  [index.md](index.md) for the canonical 18).
