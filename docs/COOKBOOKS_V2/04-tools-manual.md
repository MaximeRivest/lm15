# Cookbook 04 — Tools (Manual Loop)

Use `Tool` objects when you need full control over execution: async backends, approval gates, side effects, or multimodal tool results.

## Basic manual loop

```python
import lm15
from lm15 import FunctionTool

weather = FunctionTool(
    name="get_weather",
    description="Get weather by city",
    parameters={
        "type": "object",
        "properties": {"city": {"type": "string"}},
        "required": ["city"],
    },
)

gpt = lm15.model("gpt-4.1-mini")

# Step 1: model requests a tool call
resp = gpt.call("Weather in Montreal?", tools=[weather])

# Step 2: execute tools yourself
results = {}
for tc in resp.tool_calls:
    results[tc.id] = f"22°C, sunny in {tc.input.get('city', '?')}"

# Step 3: submit results back
resp = gpt.submit_tools(results)
print(resp.text)
```

## Stateless manual loop with `Conversation`

```python
import lm15
from lm15 import Conversation, FunctionTool

weather = FunctionTool(
    name="get_weather",
    description="Get weather by city",
    parameters={
        "type": "object",
        "properties": {"city": {"type": "string"}},
        "required": ["city"],
    },
)

conv = Conversation(system="You are helpful.")
conv.user("Weather in Montreal?")

resp = lm15.call("gpt-4.1-mini", messages=conv.messages, system=conv.system, tools=[weather])
conv.assistant(resp.response)

if resp.finish_reason == "tool_call":
    conv.tool_results({tc.id: f"22°C, sunny in {tc.input['city']}" for tc in resp.tool_calls})
    resp = lm15.call("gpt-4.1-mini", messages=conv.messages, system=conv.system, tools=[weather])
    print(resp.text)
```

## Built-in tools (provider server-side)

```python
import lm15

resp = lm15.call("gpt-4.1-mini", "Latest AI news", tools=["web_search"])
print(resp.text)

for c in resp.citations:
    print(f"  [{c.title}]({c.url})")
```

## Multi-hop tool loop

```python
import lm15
from lm15 import FunctionTool

search = FunctionTool(
    name="search",
    description="Search the web",
    parameters={"type": "object", "properties": {"query": {"type": "string"}}, "required": ["query"]},
)

agent = lm15.model("gpt-4.1-mini")
resp = agent.call("Find the population of Tokyo and the area of Japan.", tools=[search])

while resp.finish_reason == "tool_call":
    results = {}
    for tc in resp.tool_calls:
        results[tc.id] = f"Result for '{tc.input.get('query', '')}': 42 million / 377,975 km²"
    resp = agent.submit_tools(results)

print(resp.text)
```
